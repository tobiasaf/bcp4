...
def _extraer_params_markov(params_dict, regime_idx, n_exog=3):
    ...
    for k in const_keys:
        ...
        print(f"   Markov: No se encontró constante para régimen {regime_idx}. Keys: {list(params_dict.keys())[:10]}")
...
def entrenar_y_predecir_todo(
    ...
    if n_vecm_avail < n_calib:
        print(f"   VECM forecast truncado: {n_vecm_avail}/{n_calib} pasos disponibles. Padding con último valor.")
    ...
    if mae_mlp_fob > 2.5 * mediana_otros_fob:
        print(f"   MLP FOB colapsado (MAE={mae_mlp_fob:.4f} > 2.5x mediana={mediana_otros_fob:.4f}). Peso forzado a 0.")
        mae_mlp_fob = mae_mlp_fob * 1000  
    ...
    if mae_mlp_fas > 2.5 * mediana_otros_fas:
        print(f"   MLP FAS colapsado (MAE={mae_mlp_fas:.4f} > 2.5x mediana={mediana_otros_fas:.4f}). Peso forzado a 0.")
        mae_mlp_fas = mae_mlp_fas * 1000  
