...
def fetch_all_live(timeout=5):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(tasks)) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_key):
            ...
            try:
                ...
                status_flags[label] = " Online" if success else " Desconectado"
    ...
    if "rofex_precio_usd" not in unified_data:
        ...
        unified_data["rofex_volumen"] = 1200.0  
        unified_data["rofex_interes_abierto"] = 22000.0  
        status_flags["MatbaRofex"] = "ℹ Proxy (Chicago - $20)"
        
    
    if "wasde_stocks_to_use" not in unified_data:
        unified_data["wasde_stocks_to_use"] = 0.325  
        unified_data["wasde_arg_export_mt"] = 11500.0 
        if "USDA WASDE" not in status_flags or status_flags["USDA WASDE"] != " Online":
            status_flags["USDA WASDE"] = "ℹ Fallback Histórico"
            
    
    if "nivel_parana_rosario_m" not in unified_data:
        unified_data["nivel_parana_rosario_m"] = 3.5  
        status_flags["INA Paraná"] = "ℹ Fallback Histórico"
        
    
    if "nino34_anomalia" not in unified_data:
        unified_data["nino34_anomalia"] = 0.0
        unified_data["fase_enso"] = "NEUTRAL"
        status_flags["NOAA CPC ENSO"] = "ℹ Fallback Histórico"

    
    if "gtrends_vender_trigo" not in unified_data:
        unified_data["gtrends_vender_trigo"] = 50.0
        unified_data["gtrends_dolar"] = 50.0
        status_flags["Google Trends"] = "ℹ Fallback Histórico"
        
    
    if "cot_managed_money_net" not in unified_data:
        unified_data["cot_managed_money_net"] = -25000.0  
        unified_data["cot_commercial_net"] = 15000.0
        status_flags["CFTC COT Report"] = "ℹ Fallback Histórico"
